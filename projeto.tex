...
\emph{portfolio} na solução do problema do \emph{autotuning} no caso geral --- 
isto é, não restrito a problemas de decisão computacionalmente difíceis. 
...
implementadas as técnicas estudadas nos anos anteriores, que deverão ser 
testadas, através da análise de seu desempenho solução dos \emph{benchmarks}
utilizados pelo sistema PetaBricks e do \emph{benchmark} para problemas 
computacionalmente difíceis, que será elaborado como descrito na seção
\ref{sec:met}. Também serão feitas implementações de abstrações de alto nível,
que deverão encapsular as implementações dos conceitos estudados anteriormente,
e que podem tomar a forma de um arcabouço orientado ao \emph{autotuning online}
de programas, dependendo dos resultados das pesquisas e atividades realizadas 
...
\begin{center}
    ...
    >{\centering\arraybackslash}p{3cm} | 
    >{\centering\arraybackslash}p{3cm} |}
    \multicolumn{4}{c}{Anos do Doutorado Direto} \\
    \hline
    1º & 2º & 3º & 4º \\ \hline 
    \cellcolor{gray!94} Estudo das Possibilidades das Técnicas  & \cellcolor{gray!8} Implementação do algoritmo \emph{portfolio} como técnica de \emph{autotuning} & \multicolumn{2}{l|}{\cellcolor{gray!94} Aplicação de benchmarks e testes de desempenho} \\ 
    \cellcolor{gray!70} Estudo da Implementação do PetaBricks & \cellcolor{gray!32} Implementação do algoritmo \emph{portfolio} no \emph{autotuning online} & \multicolumn{2}{l|}{\cellcolor{gray!70} Implementação de abstrações de alto nível} \\ 
    \cellcolor{gray!56} Estudo da Implementação do AMF & \cellcolor{gray!56} Submissão de proposta ao BEPE & \multicolumn{2}{l|}{} \\
    \cellcolor{gray!32} Estudo da Implementação do OpenTuner & & \multicolumn{2}{l|}{} \\ 
    \cellcolor{gray!8} Estudo das Possibilidades das Técnicas do AMF &  & \multicolumn{2}{l|}{} \\
    \hline
    \end{tabular}
\end{center}
